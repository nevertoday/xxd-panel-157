...
# XXD Panel 157｜Archaeological Ink Rubbing
...
Observed limitations: some subjects are too large and retain too much scenery. Non-English lettering, red seals and unverified archival-style dates or locations appear. These are generated visual elements, not factual records; the black/grey rubbing palette and extreme whitespace are not consistently achieved. No extra generations were made.
...
- `design-only`: the full canvas contains only Panel 157's designed translation; the photograph remains a non-visible reference.
...
Restart the agent session after installation, then invoke `$xxd-panel-157`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-157 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
...
See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-157-prompt.en.md) or [Chinese](references/xxd-panel-157-prompt.zh-CN.md) runtime adapter.
